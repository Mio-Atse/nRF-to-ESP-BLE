...
#define DEVICE_NAME                     "NRF52_SPP_Server"
#define NUS_SERVICE_UUID_TYPE           BLE_UUID_TYPE_VENDOR_BEGIN                  /**< UUID type for the Nordic UART Service (vendor specific). */

#define NUS_SERVICE_UUID           BLE_UUID_NUS_SERVICE
#define NUS_CHAR_RX_UUID           BLE_UUID_NUS_RX_CHARACTERISTIC
#define NUS_CHAR_TX_UUID           BLE_UUID_NUS_TX_CHARACTERISTIC
...
static sensor_data_t sensor_data = {1, 1};
static accelerometer_data_t accel_data = {1, 1, 1};
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    {
        NRF_LOG_INFO("Received command from client");
        
        if (p_evt->params.rx_data.length == 1)
        {
            char command = p_evt->params.rx_data.p_data[0];
            uint8_t response[32];  // Increased buffer size
            ...
            switch(command)
            {
                ...
                case '2':
                    ...
                    break;
                default:
                    NRF_LOG_INFO("Unknown command received");
                    return;
            }
        ...
        case APP_UART_DATA_READY:
                ...
                (index >= m_ble_nus_max_data_len))
            {
                if (index > 1)
                {
                    ...
                    // Convert data_array to hex
                    uint8_t hex_data[2 * sizeof(data_array)];
                    uint16_t hex_data_len = 0;
                    for (uint16_t i = 0; i < index; i++)
                    {
                        hex_data_len += snprintf((char*)&hex_data[hex_data_len], sizeof(hex_data) - hex_data_len, "%02X", data_array[i]);
                    }

                    do
                    {
                        uint16_t length = hex_data_len;
                        err_code = ble_nus_data_send(&m_nus, hex_data, &length, m_conn_handle);
                        if ((err_code != NRF_ERROR_INVALID_STATE) &&
...
static void send_data_to_client(uint8_t *data, uint16_t length)
{
    uint32_t err_code;
    uint16_t len = length;
 ...
 void send_custom_message(void)
{
    uint32_t err_code;
    uint16_t length;
    uint8_t message[] = "Hello, this is a custom message!";
